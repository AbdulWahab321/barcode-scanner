...
const Input = <input />;
export default class ScanScreenWEB extends React.Component {
  constructor() {
    super();
    this.state = {
  ...
  render() {
    ...
    const barCodeScanned = async (type, data) => {
      if (
        ...
        await this.setState({
          scanned: true,
          data: "Sorry! we couldn't scan this code😥😥",
          type: type,
        });
      }
    };
    ...
    if (this.state.scanned === true) {
      if (validUrl.isUri(this.state.data)) {
        ...
        if (this.state.data === "Sorry! we couldn't scan this code😥😥") {
          return (
            <View style={styles.container}>
              <View style={styles.subContainer}>
                <Text style={[styles.displayTextS, { fontSize: 40 }]}>
                  {this.state.data}
                </Text>
                <Text style={styles.displayTextS}>Please Try On Mobile...</Text>
                <TouchableOpacity
                  style={{
                    justifyContent: 'center',
                    alignSelf: 'center',
              ...
              <View style={styles.container}>
                <View style={styles.subContainer}>
                  ...
                  <View style={{ borderWidth: 2, borderRadius: 1 }}>
                    <Text style={styles.displayTextHead}>Content : </Text>
                    <Text style={styles.displayTextS}>{this.state.data}</Text>
                    <Text style={styles.displayTextHead}>
                      Detected Code Type
                    </Text>
                    <Text style={styles.displayTextS}>{this.state.type}</Text>
                  </View>
      ...
      return (
        <View style={styles.container}>
          <View style={styles.subContainer}>
            <Text style={styles.title}>QR Code Scanner App</Text>
            <QRCodeReader
              onScan={this.state.scanned ? undefined : barCodeScanned}
            ...
            <TouchableOpacity
              ...
              style={[styles.button, { marginTop: 300 }]}>
              <Text style={styles.buttonText}>Back</Text>
            </TouchableOpacity>
          </View>
        </View>
